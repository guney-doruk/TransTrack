...
class DETRsegm(nn.Module):
    ...
    def forward_train(self, samples: NestedTensor, pre_embed=None):
        ...
        if not self.deform_detr.two_stage:
            query_embeds = self.deform_detr.query_embed.weight        
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, memory = self.deform_detr.transformer(srcs, masks, pos, query_embeds, pre_reference, pre_tgt)              
...
def dice_loss(inputs, targets, num_boxes, bbox_masking=False):
    """
...
def sigmoid_focal_loss(inputs, targets, num_boxes, batch_ignore = None, ignore_region_handling = False, alpha: float = 0.25, gamma: float = 2, bbox_masking=False):
    """
    ...
    if bbox_masking:
        ...
        prob = inputs.sigmoid()
    ##Box maskingli eğitimde cross entropye sigmoidsiz inputu koymak gerekli.
    ...
    #If the index belongs to the ignored region, the loss coming from that object should be 0 so that i would not affect the learning
    # if ignore_region_handling:
    #     for batch, unmatched_ids in batch_ignore.items():
    #         loss[batch, unmatched_ids] = 0
...
class PostProcessSegm(nn.Module):
    ...
    def forward(self, results, outputs, targets, orig_target_sizes, max_target_sizes, pred_idx, tgt_idx, bbox_masking=False):
        ##NOTE: Her ikisindende targets i çıkartabilirsin
